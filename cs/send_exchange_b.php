...
if($_POST['mode']=="allCancel"){
	if(count($_POST['chk_no'])){
		try
    	{
			...
			foreach($_POST['chk_no'] as $v){                
                ...
                if($_POST['code']=='4'){
					if($_POST['codeSelect']=='bad'){
						$CANCEL->badIns($v, 'cs_exchange_b_bad');
					}else{
						//재고등록
						$CANCEL->stockChange($v,$_POST['codeSelect'],'cs_exchange_b');
						//order_list 교환재고업데이트
						$CANCEL->cancelStock($v,'exchange_num');
						//stock_list 복원
			...
			$db->commit();
			msg("처리되었습니다.","send_exchange_b.php");
		}
		catch(Exception $e)
		{
			$db->rollBack();	
			$s = $e->getMessage() . ' (오류코드:' . $e->getCode() . ')';	
			msg($s,"send_exchange_b.php");		
		}  
	}
}
/**
