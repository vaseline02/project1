...
$self_reg=$_GET['self_reg']?$_GET['self_reg']:"0";
		...
		if (sizeof($err_msg) > 0) {
			...
			try{
				...
				foreach($excel_data as $k=>$v){
					...
					$param[':return_courier_code']='CJGLS';
					$param[':return_invoice']=str_replace("-","",$v['3']);
					$param[':courier_code']='CJGLS';
...
if($_GET){	
	...
	#================================== 주문검색조건 ===========================================
	//구매자,수령자
	if($_GET['s_mall_name']) $order_where[]="ol.mall_name like '%".$_GET['s_mall_name']."%'";
	
	if($_GET['s_buyer']){
		$middleMasking=preg_replace('/.(?=.$)/u','*',$_GET['s_buyer']); // 홍○동 
		$endMasking=preg_replace('/.(?!.)/u','*',$_GET['s_buyer']); // 홍길○
		$order_where[]="(
		ol.buyer like '".$_GET['s_buyer']."%' 
		or ol.buyer like '".$middleMasking."%' 
		or ol.buyer like '".$endMasking."%'
		)";
	}
	//if($_GET['s_buyer_mobile']) $order_where[]="ol.buyer_mobile like '%".$_GET['s_buyer_mobile']."%'";
	if($_GET['s_buyer_mobile']) $order_where[]="replace(ol.buyer_mobile,'-','') like '%".str_replace('-','',$_GET['s_buyer_mobile'])."%'";
	if($_GET['s_receiver']){
		$middleMasking=preg_replace('/.(?=.$)/u','*',$_GET['s_receiver']); // 홍○동 
		$endMasking=preg_replace('/.(?!.)/u','*',$_GET['s_receiver']); // 홍길○
		$order_where[]="(
		ol.receiver like '".$_GET['s_receiver']."%' 
		or ol.receiver like '".$middleMasking."%' 
		or ol.receiver like '".$endMasking."%'
		)";
	}
	//if($_GET['s_mobile']) $order_where[]="ol.mobile like '%".$_GET['s_mobile']."%'";
	if($_GET['s_mobile']) $order_where[]="replace(ol.mobile,'-','') like '%".str_replace('-','',$_GET['s_mobile'])."%'";
	if($_GET['s_invoice']) $order_where[]="ol.invoice like '%".$_GET['s_invoice']."%'";
	if($_GET['s_return_invoice']) $order_where[]="ol.return_invoice like '%".$_GET['s_return_invoice']."%'";
	if($_GET['s_goodsnm']) $order_where[]="ol.goodsnm like '%".$_GET['s_goodsnm']."%'";
	if($_GET['s_address']) $order_where[]="ol.address like '%".$_GET['s_address']."%'";
...
*/
	...
	if($_GET['aslist'] || $_GET['handlist'] || $viewCheck){
		...
		if($_GET['s_mall_name']) $as_where[]="ad.mall_name like '%".$_GET['s_mall_name']."%'";
		if($_GET['s_ordno']) $as_where[]="ai.order_no like '".$_GET['s_ordno']."%'";
		if($_GET['s_buyer']){
			$middleMasking=preg_replace('/.(?=.$)/u','*',$_GET['s_buyer']); // 홍○동 
			$endMasking=preg_replace('/.(?!.)/u','*',$_GET['s_buyer']); // 홍길○
			$as_where[]="(
			ai.receiver like '".$_GET['s_buyer']."%' 
			or ai.receiver like '".$middleMasking."%' 
			or ai.receiver like '".$endMasking."%'
			)";
		}
		//if($_GET['s_buyer_mobile']) $as_where[]="ai.mobile like '%".$_GET['s_buyer_mobile']."%'";
		if($_GET['s_buyer_mobile']) $as_where[]="replace(ai.mobile,'-','') like '%".str_replace('-','',$_GET['s_buyer_mobile'])."%'";
		if($_GET['s_receiver']){
			$middleMasking=preg_replace('/.(?=.$)/u','*',$_GET['s_receiver']); // 홍○동 
			$endMasking=preg_replace('/.(?!.)/u','*',$_GET['s_receiver']); // 홍길○
			$as_where[]="(
			ai.receipt_name like '".$_GET['s_receiver']."%' 
			or ai.receipt_name like '".$middleMasking."%' 
			or ai.receipt_name like '".$endMasking."%'
			)";
		}
		//if($_GET['s_mobile']) $as_where[]="ai.mobile like '%".$_GET['s_mobile']."%'";
		if($_GET['s_mobile']) $as_where[]="replace(ai.mobile,'-','') like '%".str_replace('-','',$_GET['s_mobile'])."%'";
		if($_GET['s_invoice']) $as_where[]="ad.invoice like '%".$_GET['s_invoice']."%'";
//		if($_GET['s_return_invoice']) $as_where[]="ai.return_invoice like '%".$_GET['s_return_invoice']."%'";
		if($_GET['s_goodsnm']) $as_where[]="ad.goodsnm like '%".$_GET['s_goodsnm']."%'";
...
*/
		if($as_where || $order_where || $_GET['aslist'] || $_GET['handlist']){
			if($order_where || $as_where)   $as_sum_where=" and (";
			if($order_where)				$as_sum_where.="(".implode(" and ", $order_where).")";
			if($order_where && $as_where)   $as_sum_where.=" or ";
			if($as_where)					$as_sum_where.="(".implode(" and ", $as_where).")";
			if($order_where || $as_where)   $as_sum_where.=")";
	...
	if($_GET['cslist'] || $_GET['returnlist'] || $viewCheck){
		...
		if($_GET['s_mall_name']) $cs_where[]="ol.mall_name like '%".$_GET['s_mall_name']."%'";
		if($_GET['s_ordno']) $cs_where[]="ci.order_no like '".$_GET['s_ordno']."%'";
		/*
		if($_GET['s_buyer']){
			$middleMasking=preg_replace('/.(?=.$)/u','*',$_GET['s_buyer']); // 홍○동 
			$endMasking=preg_replace('/.(?!.)/u','*',$_GET['s_buyer']); // 홍길○
			$cs_where[]="(
			ci.receiver like '".$_GET['s_buyer']."%' 
			or ci.receiver like '".$middleMasking."%' 
			or ci.receiver like '".$endMasking."%'
			)";
		}
		*/
		//if($_GET['s_buyer_mobile']) $cs_where[]="ci.mobile like '%".$_GET['s_buyer_mobile']."%'";
		if($_GET['s_buyer_mobile']) $cs_where[]="replace(ci.mobile,'-','') like '%".str_replace('-','',$_GET['s_buyer_mobile'])."%'";

		if($_GET['s_receiver']){
			$middleMasking=preg_replace('/.(?=.$)/u','*',$_GET['s_receiver']); // 홍○동 
			$endMasking=preg_replace('/.(?!.)/u','*',$_GET['s_receiver']); // 홍길○
			$cs_where[]="(
			ci.receiver like '".$_GET['s_receiver']."%' 
			or ci.receiver like '".$middleMasking."%' 
			or ci.receiver like '".$endMasking."%'
			)";
		}
		//if($_GET['s_mobile']) $cs_where[]="ci.mobile like '%".$_GET['s_mobile']."%'";
		if($_GET['s_mobile']) $cs_where[]="replace(ci.mobile,'-','') like '%".str_replace('-','',$_GET['s_mobile'])."%'";
		//if($_GET['s_invoice']) $cs_where[]="ci.invoice like '%".$_GET['s_invoice']."%'";
		if($_GET['s_return_invoice']) $cs_where[]="ci.return_invoice like '%".$_GET['s_return_invoice']."%'";
		if($_GET['s_goodsnm']) $cs_where[]="cd.exchange_goods_nm like '%".$_GET['s_goodsnm']."%'";
...
*/
		if($cs_where || $order_where || $_GET['cslist'] || $_GET['returnlist']){
			if($order_where || $cs_where)   $cs_sum_where=" and (";
			if($order_where)				$cs_sum_where.="(".implode(" and ", $order_where).")";
			if($order_where && $cs_where)   $cs_sum_where.=" or ";
			if($cs_where)					$cs_sum_where.="(".implode(" and ", $cs_where).")";
			if($order_where || $cs_where)   $cs_sum_where.=")";

			//$cs_where=" and ".implode(" and ", $cs_where);
			...
			if($_GET['cslist'] && $_GET['returnlist']){
				...
				if($_GET['returnlist']){
					if(!$_GET['self_reg']){
						...
						$cs_qry.=" and ci.return_type in('60','70','90')";
					}
				}
			}
			...
			if($_GET['returnlist']){
				if($_GET['self_reg']){
					...
					//admin_no complete_yn return_type  1,2,4
					$self_reg_qry="select order_list_no from cs_receipt where receipt_type='0' and return_type in('1','2','4') and complete_yn='N' and admin_no='".$_SESSION["sess"]["m_no"]."'";
					$self_reg_res=$db->query($self_reg_qry);
					unset($self_reg_no);
					foreach( $self_reg_res->results as $self_v ){
						$self_reg_no[]=$self_v['order_list_no'];
					}
					
					tydebug($self_reg_no);
					$cs_qry.=" and cd.order_list_no in ('".implode("','",$self_reg_no)."')";
					//$cs_qry.=" and ci.admin_no='".$sess['m_no']."' and ci.add_type='2'";
				}
			}
	...
	if($viewCheck){
		if($_GET['s_ordno']) $order_where[]="ol.ordno like '".$_GET['s_ordno']."%'";
		if($order_where) $order_sum_where.=" and (".implode(" and ", $order_where).")";
		
		$order_qry="select ol.ordno from order_list ol	
		where 1=1 ".$order_sum_where."";
		if($_GET['s_date'] && $_GET['e_date']){
	...
	(select 
			...
			,comp_date as comp_date, bundle as bundle ,deli_codeno as deli_codeno ,courier_code as courier_code
			...
			,ol.step, ol.step2 ,ol.upload_form_type
			,if(g.goodsnm,g.goodsnm,ol.goodsnm) as goodsnm
			,b.brand_img_folder
		...
		select 
			...
			,'' as comp_date, '' as bundle ,'' as deli_codeno,'' as courier_code
			...
			,'' as step, '' as step2, '' as upload_form_type
			,ad.goodsnm
			,'' as brand_img_folder
	...
	foreach($res->results as $v){
		...
		$v['comp_date']=($v['comp_date']!='0000-00-00 00:00:00' && $v['step2']<40)?$v['comp_date']:"";
...
$rqry="select cr.route, cr.no as receipt_no, cr.memo, ol.mall_name, ol.receiver, ol.goodsnm, ol.ordno, ol.mall_no, ol.no, m.name, m.id from cs_receipt cr
...
foreach($rres->results as $rv){
	...
	$rv = infoMasking($rv, 'order_list'); //마스킹
...
$checked['self_reg'][$self_reg]="checked";
