...
function get_search_where(){
	...
	if($_REQUEST['s_b_paste']){
		$s_b_paste=paste_to_arr($_REQUEST['s_b_paste']);
		if($s_b_paste){
			$s_b_paste_imp=implode("','",$s_b_paste);
			$where[]="b.brandnm in ('".$s_b_paste_imp."')";	
		}		
	}
	...
	//카테고리 미적용
	if($_REQUEST['s_no_cate']){
		$where[]="(select count(1) from goods_link where goodsno=g.no)=0";
		$checked['s_no_cate']['1']='checked';
	}

	if($_REQUEST['s_brand'])$where[]="b.brandnm like '%".$_REQUEST['s_brand']."%'";

	$_REQUEST['s_model']=trim($_REQUEST['s_model']);
	if(strpos($_SERVER['PHP_SELF'], 'goods_list.php') !== false) {  
		if($_REQUEST['s_model']){
			$where[]="(g.goodsnm like '%".$_REQUEST['s_model']."%' or g.goodsnm_sub like '%".$_REQUEST['s_model']."%')";
		}
	} else {  
	...
	If($_REQUEST['s_img']){
		if($_REQUEST['s_img']=="1"){
			$where[]="g.img_step in('4','5')";
		}else if($_REQUEST['s_img']=="2"){
			$where[]="g.img_step in('1','2','3')";
		}
		$checked['s_img'][$_REQUEST['s_img']]='checked';
	}else{
		$checked['s_img']['']='checked';
	}

	If(is_numeric($_REQUEST['s_psb_stock']) || is_numeric($_REQUEST['e_psb_stock'])){
		if($_REQUEST['s_psb_stock'])$where[]="(".implode('+',$psb_stock_loc).") >= '".$_REQUEST['s_psb_stock']."'";
		if($_REQUEST['e_psb_stock'])$where[]="(".implode('+',$psb_stock_loc).") <= '".$_REQUEST['e_psb_stock']."'";
	}
...
function get_codedata($type='',$v2='',$no=''){
	...
	if($no){
		...
		if(is_array($no)){
			...
			$where.=" and no in (".implode(",",array_keys($inparam)).")";

			$param=array_merge($param,$inparam);
		}else{
			$where.=" and no=:no";
			$param[':no']=$no;
		}
	...
	$qry="select no,type,cd,v,v2,v3,place_type,place_code,view_yn from codedata where 1 ".$where." ".$orderby;
...
//place_type별 불러오기
function get_codedata_type($type='',$v2='',$no='', $place_type='', $view_yn=''){
	...
	$qry="select no,type,cd,v,v2,v3,place_type,place_code,view_yn from codedata where place_type!='' ".$where." ".$orderby;
    $res=$db->query($qry,$param);
...
$arr_io_type['order_list']=array('column'=>'no','type'=>array('reserve','order','order_cancel','order_step_back','order_d_hold'));
$arr_io_type['cs_detail']=array('column'=>'order_list_no','type'=>array('cs_hold','cs_hold_cancel','cs_return','cs_exchange','cs_exchange_b','re_order','repairing','cs_send'));
...
function get_mall_group(){
	global $db;
	$qry="select ml.upload_form_type from mall_list ml where ml.upload_form_type!='' group by ml.upload_form_type 
	order by 
	CASE
...
function get_mall_info($state='',$no='',$group_id=''){
	...
	if($group_id){
		...
		$param[':upload_form_type']=$group_id;
	}
...
function sms_send_ppurio($title, $contents, $mobile, $etc_code='', $etc_no=0, $send_number=0, $order_no=''){
    global $db;
	...
	$_param['userid'] = 'trendmecca';           // [필수] 뿌리오 아이디
	$_param['callback'] = '15998246';    // [필수] 발신번호 - 숫자만
	$_param['phone'] = $mobile;       // [필수] 수신번호 - 여러명일 경우 |로 구분 '010********|010********|010********'
	$_param['msg'] = $contents;   // [필수] 문자내용 - 이름(names)값이 있다면 [*이름*]가 치환되서 발송됨
	//$_param['names'] = '홍길동';            // [선택] 이름 - 여러명일 경우 |로 구분 '홍길동|이순신|김철수'
	//$_param['appdate'] = '20190502093000';  // [선택] 예약발송 (현재시간 기준 10분이후 예약가능)
	$_param['subject'] = $title;          // [선택] 제목 (30byte)
...
function goods_freegift_wholesale(){ //도매 사은품
	
	global $db;
	
	$qry="select gf.*,g.goodsnm giftnm from wholesale_goods_freegift gf
	join goods g on gf.gift_goodsno = g.no
	where gf.view_yn='Y'
	";
	$res=$db->query($qry);
...
function get_goods_gift($v,$k,$giftinfo){
	...
	if($data){
		...
		foreach($data as $giftv){
			...
			$val[$k."_".$loop_gift]['dupl_key']="gift_".$v['goodsnm']."_".$loop_gift;
			$loop_gift++;
		}
	}
...
function wholesale_payment_ins($ptype, $ordno_seq, $num, $price){
	global $db;

	$sqry="select wno from order_list where no='".$ordno_seq."'";
	$sres=$db->query($sqry);
	$orderData=$sres->results[0];

	if($orderData['wno']){
		$qry="insert into wholesale_payment set
		ptype='".$ptype."'
		,worder_seq='".$orderData['wno']."'
		,ordno_seq='".$ordno_seq."'
		,num='".$num."'
		,price='".$price."'
		,reg_m_no='".$_SESSION["sess"]["m_no"]."'
		,reg_date=now()
		";
		$db->query($qry);
	}
...
function codedata_viewchg_log($no, $view_yn){
	global $db;

	$qry="select view_yn from codedata where no='".$no."'";
	$res=$db->query($qry);
	$codeData=$res->results[0];

	if($codeData['view_yn']!=$view_yn){
		$iqry="insert into codedata_view_log set
		codeno='".$no."'
		,m_no='".$_SESSION['sess']['m_no']."'
		,b_view_yn='".$codeData['view_yn']."'
		,a_view_yn='".$view_yn."'
		,reg_date=now()
		";
		$db->query($iqry);
...
function letterMasking($_type, $_data, $_date=''){
    /* $_type 값 정의
     * N : 이름
     * P : 전화번호
     * B : 생일
	...
	if($_SESSION['sess']['m_no']!="69" && $_SESSION['sess']['m_no']!="142" && $_SESSION['sess']['m_no']!="33"){
		return $_data;
	}
...
_day   : 마스킹제외할 일수
**/
function infoMasking($_value, $_type, $_day='7'){
	//마스킹 제외 회원 시퀀스
	//$maskingPassMno=array('69','142','33'); 
	$maskingPassMno=array(''); 

	//마스킹 항목
	$maskingName=array('buyer','receiver','receipt_name', 'name1', 'name2'); //이름 컬럼
	$maskingMobile=array('buyer_mobile','mobile', 'mobile1', 'mobile2'); //전화번호 컬럼
	$maskingAddress=array('address'); //주소 컬럼
	$maskingBirthday=array(''); //생일 컬럼
	$maskingIp=array(''); //ip 컬럼
	$maskingEmail=array('email'); //이메일 컬럼
	$maskingAccount=array(''); //계좌번호 컬럼
	$maskingThree=array(''); //3글자이상 마스킹 컬럼
	$maskingAll=array(''); //전체마스킹


	$tableDate = date('Y-m-d', strtotime($_value['reg_date']));
	$checkDate = date('Y-m-d', strtotime("-".$_day." days") );

	if($checkDate<$tableDate || (in_array($_SESSION['sess']['m_no'],$maskingPassMno))){
		return $_value;
	}

	//임시로 개발자 제외하고 마스킹안된상태로 보이게 처리
	if($_SESSION['sess']['m_no']!="69" && $_SESSION['sess']['m_no']!="142" && $_SESSION['sess']['m_no']!="33"){
		return $_value;
	}

	//이름 마스킹
	foreach($maskingName as $mv){
		if(array_key_exists($mv,$_value)){
			$strlen = mb_strlen($_value[$mv], 'utf-8');
			$maskingValue = "";
			$maskingCut="";
			$useHyphen = "-";
			...
			switch($strlen){
				case 1:
					$maskingValue = mb_substr($_value[$mv], 0, 1, "UTF-8");
					break;
				case 2:
					$maskingValue = mb_substr($_value[$mv], 0, 1, "UTF-8").'*';
					break;
				default:
					...
					$maskingValue = mb_substr($_value[$mv], 0, 1, "UTF-8").$maskingCut.mb_substr($_value[$mv], $strlen-1, 1, "UTF-8");
					break;
			}
			$_value[$mv]=$maskingValue;
		}
	}
	
	//전화번호 마스킹
	foreach($maskingMobile as $mv){
		if(array_key_exists($mv,$_value)){
			$strlen = mb_strlen($_value[$mv], 'utf-8');
			$maskingValue = "";
			$maskingCut="";
			...
			$explode_data=explode('-',$_value[$mv]);
			if(count($explode_data)>'1'){
				$number_strlen = mb_strlen($explode_data[1], 'utf-8');
				for($i=0;$i<$number_strlen;$i++){
					$maskingCut.="*";
				...
				switch($strlen){
					case 9:
						//$maskingValue = mb_substr($_data, 0, 2, "UTF-8")."{$useHyphen}***{$useHyphen}*".mb_substr($_data, $strlen-3, 4, "UTF-8");
						$maskingValue = mb_substr($_value[$mv], 0, 2, "UTF-8")."{$useHyphen}***{$useHyphen}".mb_substr($_value[$mv], $strlen-4, 4, "UTF-8");
						break;
					default:
						//$maskingValue = mb_substr($_data, 0, 3, "UTF-8")."{$useHyphen}****{$useHyphen}*".mb_substr($_data, $strlen-3, 4, "UTF-8");
						$maskingValue = mb_substr($_value[$mv], 0, 3, "UTF-8")."{$useHyphen}****{$useHyphen}".mb_substr($_value[$mv], $strlen-4, 4, "UTF-8");
						break;
				}
			}
			$_value[$mv]=$maskingValue;
	...
	foreach($maskingAddress as $mv){
		if(array_key_exists($mv,$_value)){
			...
			if(count($explode_data)>'1'){
				...
				$maskingValue = mb_substr($_value[$mv], 0, 10, "UTF-8")." ".$maskingCut;
			}
			$_value[$mv]=$maskingValue;
		}
	}
	...
	foreach($maskingBirthday as $mv){
		if(array_key_exists($mv,$_value)){
			...
			$maskingValue = mb_substr($_value[$mv], 0, 2, "UTF-8")."**{$useHyphen}**{$useHyphen}**";
	...
	foreach($maskingIp as $mv){
		if(array_key_exists($mv,$_value)){
			...
			if(count($explode_data)>'1'){
				...
				$maskingValue = mb_substr($_value[$mv], 0, 6, "UTF-8")." ".$maskingCut;
			}
	...
	foreach($maskingEmail as $mv){
		if(array_key_exists($mv,$_value)){
			...
			if(count($explode_data)>'1'){
				...
				$maskingValue = mb_substr($_value[$mv], 0, 2, "UTF-8").$maskingCut."@".$explode_data[1];
			}else {
				for($i=0;$i<($strlen-2);$i++){
					$maskingCut.="*";
				}
				$maskingValue = mb_substr($_value[$mv], 0, 2, "UTF-8")." ".$maskingCut;
			}
	...
	foreach($maskingAccount as $mv){
		if(array_key_exists($mv,$_value)){
			...
			if(count($explode_data)>'1'){
				...
				$maskingValue = mb_substr($_value[$mv], 0, 8, "UTF-8").$maskingCut;
			}
	...
	foreach($maskingThree as $mv){
		if(array_key_exists($mv,$_value)){
			...
			$maskingValue = mb_substr($_value[$mv], 0, 3, "UTF-8").$maskingCut;
