	...
	if(count($excel_data)>0){
		...
		if (sizeof($err_msg) > 0) {
			...
			try{
				...
				//$db->rollBack();
                $db->commit();
	            msg('처리되었습니다.'.$dmsg,'order_final_chk.php?'.$QUERY_STRING);
...
if($_POST['mode']=='order_settle' ){
	...
	msg($mag,'order_final_chk.php?'.$QUERY_STRING);
	...
	try{
		...
		foreach($_POST['chk_no'] as $v){
			...
			$stock_deli_av=$GOODS->get_stock_deli_av(array($_POST['hid_goodsnm'][$v]),$_POST['place_code']);
			
			if($stock_deli_av->results['0']['totstock']>=$_POST['hid_ord_num'][$v]){
				$comp_cnt++;
				...
				$db->query($qry,array(":no"=>$v,":deli_codeno"=>$_POST['place_code']));
				$okd=stock_io('order',$goodsno,$_POST['hid_goodsnm'][$v],-$_POST['hid_ord_num'][$v],$v,$_SERVER['REQUEST_URI'],$_POST['place_code']);
		...
		//$db->rollBack();
		$db->commit();
		msg($comp_cnt."건 처리되었습니다","order_final_chk.php");		
	}
	catch( Exception $e ){
		tydebug('err');
		$db->rollBack();
	...
	*/
}else if($_POST['mode']=='cp_order' ){ //출고 불가 제품 주문 복사 기능
	...
	try{
		...
		foreach($_POST['chk_no'] as $v){
			...
			if($_POST['hid_ord_num'][$v]>$_POST['cp_num'] && $_POST['cp_num']){
				$comp_cnt++;
		...
		//$db->rollBack();
		$db->commit();
		msg($comp_cnt."건 처리되었습니다","order_final_chk.php?".$QUERY_STRING);		
	}
	catch( Exception $e ){
		tydebug('err');
		$db->rollBack();
	...
	try{
		...
		//$ORDER->ctl_step($_POST['chk_no'],'1');//기존소스 삭제
		...
		//$db->rollBack();
		$db->commit();
		if($err_msg)msg(implode(" ",$err_msg),'order_final_chk.php?'.$QUERY_STRING);
		else msg("처리되었습니다","order_final_chk.php?".$QUERY_STRING);		
	}
	catch( Exception $e ){
		tydebug('err');
		$db->rollBack();
	...
	try{
		...
		$db->commit();
		msg('처리되었습니다.','order_final_chk.php?'.$QUERY_STRING);
