	...
	if(count($excel_data)>0){
		...
		if (sizeof($err_msg) > 0) {
			...
			try{
				...
				//$db->rollBack();
                $db->commit();
	            msg('처리되었습니다.'.$dmsg,'order_stock_shortage_stand.php?'.$QUERY_STRING);
...
if($_POST['chk_no'] ){
	...
	if($_POST['mode']=='deli'){
		try{
			...
			foreach($stock_deli_av->results as $v){
				...
				if($arr_stock_chk[$v['goodsnm']]<=$v['totstock']){
					...
					$msg.=" ".$v['goodsnm']."( 발송확인단계 ".number_format($f_chk[$v['goodsnm']])."개 있음) ";
				}
			}
			...
			//$db->rollBack();
			$db->commit();
			msg('처리되었습니다.'.$msg,'order_stock_shortage_stand.php?'.$QUERY_STRING);
	...
	}else if($_POST['mode']=='cancel_stand'){
		...
		,step2=0
		,step_fixed=1
		where step=2 and step2=1
		and no in (".implode(",",array_keys($param)).") 
		";
		
		$db->query($qry,$param);
		msg('처리되었습니다.','order_stock_shortage_stand.php?'.$QUERY_STRING );
		...
		try{
			...
			msg($mag,'order_stock_shortage_stand.php?'.$QUERY_STRING);
...
foreach($res->results as $v){
	...
	$chk_stand_stock[$v['goodsnm']]['s_cnt']=$v['codeno_3'];
}

foreach($chk_stand_stock as $k=>$v){
	if($v['cnt']>$v['s_cnt']){
		$over_stand_stock[$k]=$v;
	}
}
...
,'nav_view' => $nav_view
,'over_stand_stock' => $over_stand_stock
));
