<?php /* Template_ 2.2.8 2020/12/15 15:01:49 /www/html/ukk_test2/data/skin/cs/cs_total_list.htm 000019900 */ 
$TPL_loop_1=empty($TPL_VAR["loop"])||!is_array($TPL_VAR["loop"])?0:count($TPL_VAR["loop"]);
$TPL_rloop_1=empty($TPL_VAR["rloop"])||!is_array($TPL_VAR["rloop"])?0:count($TPL_VAR["rloop"]);?>
...
<table class="fileload-list-small" >
<colgroup>
	<col width="75%" />
	<col width="25%" />
</colgroup>
...
<?php if($GLOBALS["print_xls"]!= 1){?>
		...
		<div class="row">
			...
			<div class="col-lg-12">
				...
				<form enctype="multipart/form-data" method="post" onsubmit="return chkform();">
				<input type="hidden" name="mode_return_invoice" value="1">
				<div class="panel panel-default panel-stock margin20">
					<table class="table fileload-list-small" >
						<tr>
							...
							<td>
								<input type="file" name="excelFile[]" required/><button class="btn btn-primary" >업로드</button>
								<!--
								<button type="button" class="btn btn-success" onclick="location.href='../xls_file/return_inv.xls'">양식 다운로드</button>
								-->
							</td>
						</tr>
					</table>
				...
				<div class="panel panel-default panel-stock margin20">
					<table class="table fileload-list-small">
						...
						<tbody>
							...
							<tr>
								<th scope="row" class="align-left">쇼핑몰</th>
								<td class="receive-title no-gutters">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_mall_name" value="<?php echo $_GET['s_mall_name']?>"></div>
								</td>
							...
							<tr>
								<th scope="row" class="align-left">주문자</th>
								<td class="receive-title no-gutters">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_buyer" value="<?php echo $_GET['s_buyer']?>"></div>
								</td>
								<th scope="row" class="align-left">주문자연락처</th>
								<td class="receive-title no-gutters">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_buyer_mobile" value="<?php echo $_GET['s_buyer_mobile']?>"></div>
								</td>
							</tr>
							<tr>
								<th scope="row" class="align-left">수령자/고객명</th>
								<td class="receive-title no-gutters">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_receiver" value="<?php echo $_GET['s_receiver']?>"></div>
								</td>
								<th scope="row" class="align-left">수령자연락처</th>
								<td class="receive-title no-gutters">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_mobile" value="<?php echo $_GET['s_mobile']?>"></div>
								</td>
							</tr>
							<tr>
								<th scope="row" class="align-left">출고송장번호</th>
								<td class="receive-title no-gutters">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_invoice" value="<?php echo $_GET['s_invoice']?>"></div>
								</td>
								<th scope="row" class="align-left">반송장번호</th>
								<td class="receive-title no-gutters">
							...
							<tr>
								...
								<th scope="row" class="align-left">기타검색</th>
								<td class="receive-title no-gutters">
									...
									<div class="form-check form-check-inline">
										...
										(&nbsp; <input type="checkbox" name="self_reg" id="self_reg" value=1 <?php echo $GLOBALS["checked"]['self_reg']['1']?>><label class="form-check-label" for="self_reg">직접접수 처리중</label>&nbsp;)
									</div>
									<div class="form-check form-check-inline">
							...
							<tr>
								<th scope="row" class="align-left">주소검색</th>
								<td class="receive-title no-gutters" colspan="3">
									<div class="col-xs-12"><input type="text" class="form-control" name="s_address" value="<?php echo $_GET['s_address']?>"></div>
								</td>
...
<?php if($TPL_V1["returnCheck"]){?><div style="padding:0px 3px 0px 3px; color:#fff; background-color: #f04ee8; font-size: 8px; text-align: center;border-radius: 4px;">교/반</div><?php }?>
									...
									<td><?php if(($TPL_V1["step"]=='5'||$TPL_V1["step"]=='51')&&$TPL_V1["courier_code"]&&$TPL_V1["invoice"]){?><a href="<?php echo $GLOBALS["delivery_list"][$TPL_V1["courier_code"]]['tracking']?><?php echo $TPL_V1["invoice"]?>" target="_blank"><?php echo $TPL_V1["step_lv"]?><?php if($TPL_V1["invoice"]){?><br><?php echo $TPL_V1["invoice"]?><?php }?></a><?php }else{?><?php echo $TPL_V1["step_lv"]?><?php }?></td>
...
<?php }?>
	...
	<td style="vertical-align:top;">
		<div class="row">
			<div class="col-lg-12">
				<div class="common-table-wrapper" style="padding:10px;">								
					<table id="" class="table common-table display display_dt">
<?php if($GLOBALS["print_xls"]!= 1){?>
						<colgroup>					
						</colgroup>
<?php }?>
						...
						<tbody>
<?php if($TPL_rloop_1){foreach($TPL_VAR["rloop"] as $TPL_V1){?>
						
							<tr onclick="popup('cs_reg.php?route=2&receipt_no=<?php echo $TPL_V1["receipt_no"]?>&ordno=<?php echo $TPL_V1["ordno"]?>&mall_no=<?php echo $TPL_V1["mall_no"]?>&order_seq=<?php echo $TPL_V1["no"]?>','','1100','900')">			
								<td <?php if($TPL_V1["route"]=='2'){?>style="background-color:#28992C;opacity: 0.7;"<?php }?>><?php echo $TPL_V1["mall_name"]?></td>
								<td <?php if($TPL_V1["route"]=='2'){?>style="background-color:#28992C;opacity: 0.7;"<?php }?>><?php echo $TPL_V1["receiver"]?> </td>
								<td <?php if($TPL_V1["route"]=='2'){?>style="background-color:#28992C;opacity: 0.7;"<?php }?>><?php echo $TPL_V1["goodsnm"]?></td>							
								<td <?php if($TPL_V1["route"]=='2'){?>style="background-color:#28992C;opacity: 0.7;"<?php }?>><?php echo $TPL_V1["name"]?><br>(<?php echo $TPL_V1["id"]?>)</td>							
								<td <?php if($TPL_V1["route"]=='2'){?>style="background-color:#28992C;opacity: 0.7;"<?php }?>><?php echo $TPL_V1["memo"]?></td>							
							</tr>
<?php }}?>
						</tbody>
					</table>
