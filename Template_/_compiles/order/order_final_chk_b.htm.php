<?php /* Template_ 2.2.8 2021/03/17 16:25:37 /www/html/ukk_test2/data/skin/order/order_final_chk_b.htm 000007399 */ 
...
<?php if($GLOBALS["print_xls"]!= 1){?><td><input type="checkbox" class="chk_no chk_no_<?php echo $TPL_V1["ordno"]?>" data-ordno="<?php echo $TPL_V1["ordno"]?>" name="chk_no[]" value="<?php echo $TPL_V1["no"]?>"></td><?php }?>
...
<fieldset class="page_field_info">
  ...
  <ul>
	...
	3.주문수량때문에 한곳에서 발송 못하는 주문은 좌측하단 주문분리복제 기능으로 분리후 배송위치 '통합발송' 설정후 발송확정.<br/>
	※묶음 주문의 특성상 묶음 주문 전부가 있어야만 발송가능
	</li>
	...
	<li>통합발송-묶음 배송의 각 상품들의 보유재고 위치가 다를경우 각각 발송가능 위치에서 발송(재고차감)</li>
  </ul>
</fieldset>
...
$(function(){
	...
	$(".chk_no").click(function(){
		...
		var chk_order=$(this).data("ordno");
		var this_chked=$(this).prop("checked");

		$(".chk_no_"+chk_order).prop("checked",this_chked);
	});
	*/
