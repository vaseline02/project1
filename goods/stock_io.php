...
/*
	...
	*/
	/*
    $qry="select sil.*, c.cd as codename 
	,(select ordno from order_list where no=sil.reference_no) ordno
	,(select upload_form_type from order_list where no=sil.reference_no) upload_form_type
	from stock_io_log sil
    left join codedata c on (sil.loc_f=c.no)
    where ".implode(" and ", $add_where)."
	".$add_having."
